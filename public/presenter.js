...
function render(s){
  ...
  if(s.state==='lobby'){
    ...
    screen.innerHTML=`<div class="lobby-grid"><div class="qr-column"><h1 class="lobby-title">G IOTK</h1><img class="qr" src="/api/qr/${code}" alt="Código QR de acceso"></div><div class="join-details"><div class="access-label">Código de acceso</div><div class="code">${code}</div><p class="muted">Si no puedes leer el QR, ingresa a:</p><div class="join-url">${esc(joinUrl)}</div><p class="muted">y escribe el código de acceso.</p></div></div><h3>${s.joinedCount} jugador${s.joinedCount===1?'':'es'} conectado${s.joinedCount===1?'':'s'}</h3><div class="lobby-actions"><a href="/admin" class="btn alt" style="text-decoration:none">Administración</a><button id="next" class="btn good" ${s.joinedCount<1?'disabled':''}>Iniciar primera pregunta</button></div>`;
