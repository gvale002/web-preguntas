...
function render(s){
  state=s;const quizBadge=document.querySelector('#presenterQuizBadge');if(quizBadge)quizBadge.textContent=s.quizTitle||'';if(musicOn&&!soundUnlocked)showSoundGate();setAudioForState(s.state);
  if(s.state==='lobby'){
    ...
    const q=s.question;screen.innerHTML=`<div class="badge">Pregunta ${s.currentQuestion+1} de ${s.totalQuestions}</div><div class="question-stage ${q.image?'has-image':''}">${q.image?`<div class="question-media"><img src="${q.image}" alt="Imagen de apoyo de la pregunta"></div>`:''}<div class="question-content"><h1 class="big question-title">${esc(q.text)}</h1><div class="timer"><div id="bar"></div></div><div id="secs" class="big timer-number">${Math.ceil(s.remainingMs/1000)}</div><div class="answers">${q.type==='text'?'<div class="answer">Respuesta libre desde cada dispositivo</div>':q.options.map((x,i)=>`<div class="answer">${String.fromCharCode(65+i)} · ${esc(x)}</div>`).join('')}</div></div></div><p class="muted">Respuestas recibidas en tiempo real · ${s.joinedCount} jugadores</p><button id="end" class="btn danger">Terminar pregunta ahora</button>`;document.querySelector('#end').onclick=()=>action('/api/presenter/end-question');tick();
