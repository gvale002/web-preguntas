...
function render(s){
  state=s;screen.classList.toggle('question-active',s.state==='question');if(s.state!=='question')document.querySelector('.presenter-top').style.marginBottom='24px';const quizBadge=document.querySelector('#presenterQuizBadge');if(quizBadge)quizBadge.textContent=s.quizTitle||'';if(musicOn&&!soundUnlocked)showSoundGate();setAudioForState(s.state);
  if(s.state==='lobby'){
