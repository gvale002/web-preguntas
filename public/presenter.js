...
document.querySelector('#music').textContent=`♫ Música: ${musicOn?'ON':'OFF'}`;
document.querySelector('#music').onclick=()=>{musicOn=!musicOn;localStorage.setItem('giotk-music',musicOn?'on':'off');document.querySelector('#music').textContent=`♫ Música: ${musicOn?'ON':'OFF'}`;if(!musicOn)audioEngine.stop();else{unlockSound();if(state)setAudioForState(state.state),audioEngine.set(lastAudioState);}};
